...
>  PS:只有调用 `ApplicationContext#start` 才会发送 `ContextStartedEvent`。若不想这么麻烦，可以监听 `ContextRefreshedEvent` 事件代替。一旦 Spring [容器](https://cloud.tencent.com/product/tke?from=10680)初始化完成，就会发送 `ContextRefreshedEvent`。 
